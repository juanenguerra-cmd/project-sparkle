...
export const canonicalizeIPRow = (
  ...
  for (const field of IP_CENTRALIZED_COLUMNS) {
    ...
    normalized[field] = DATE_ONLY_FIELDS.has(field) ? normalizeDate(String(raw || '')) : raw;
  }
  ...
  const createdAt = getIPFieldValue(row, 'createdAt');
  if (createdAt !== undefined) {
    normalized.createdAt = String(createdAt);
  }

  const updatedAt = getIPFieldValue(row, 'updatedAt');
