...
import { nowISO, canonicalMRN, isoDateFromAny, todayISO } from './parsers';
...
// - no end date (even if status is blank)
...
export const getActiveABT = (db: ICNDatabase): ABTRecord[] => {
  ...
  return db.records.abx.filter(r => {
    ...
    // Match ABT View logic: active if status='active', no endDate, or endDate in the future
