  ...
  return (
    <div id="report-content" className="binder-report bg-white text-black p-6">
      ...
      <section className="binder-page space-y-4">
        ...
        <table className="w-full text-sm border-collapse border">
          <thead><tr><th>Resident Name</th><th>Room</th><th>Onset Date</th><th>Status</th><th>Notes</th></tr></thead>
          <tbody>{data.outbreakCases.length === 0 ? renderEmptyState(5) : data.outbreakCases.map((row, index) => <tr key={`outbreak-case-${index}`}><td>{row.residentName}</td><td>{row.room}</td><td>{row.onsetDate}</td><td>{row.status}</td><td>{row.notes}</td></tr>)}</tbody>
        </table>
        ...
        <table className="w-full text-sm border-collapse border"><thead><tr><th>Room</th><th>Resident Name</th><th>Prophylaxis Type</th><th>Start Date</th><th>Status</th><th>Notes</th></tr></thead><tbody>{data.prophylaxis.length === 0 ? renderEmptyState(6) : data.prophylaxis.map((row, index) => <tr key={`prophylaxis-${index}`}><td>{row.room}</td><td>{row.residentName}</td><td>{row.prophylaxisType}</td><td>{row.startDate}</td><td>{row.status}</td><td>{row.notes}</td></tr>)}</tbody></table>
        ...
        <table className="w-full text-sm border-collapse border"><thead><tr><th>Room</th><th>Resident Name</th><th>Shared Equipment</th><th>Cleaning Status</th><th>Notes</th></tr></thead><tbody>{data.sharedEquipment.length === 0 ? renderEmptyState(5) : data.sharedEquipment.map((row, index) => <tr key={`shared-equipment-${index}`}><td>{row.room}</td><td>{row.residentName}</td><td>{row.equipment}</td><td>{row.cleaningStatus}</td><td>{row.notes}</td></tr>)}</tbody></table>
        ...
        <table className="w-full text-sm border-collapse border"><thead><tr><th>Focus Area</th><th>Reminder</th><th>Owner</th></tr></thead><tbody>{data.ppeFocus.length === 0 ? renderEmptyState(3) : data.ppeFocus.map((row, index) => <tr key={`ppe-${index}`}><td>{row.focusArea}</td><td>{row.reminder}</td><td>{row.owner}</td></tr>)}</tbody></table>
